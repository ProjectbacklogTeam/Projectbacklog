...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if (strlen($topic) == 0 || strlen($email) == 0 || strlen($detail) == 0) {
            ...
            $stmtfile = $db->prepare('SELECT file_prototype FROM prototype WHERE excute_id = ?');
            $stmtfile->bindParam(1, $data->idexcute);
            $stmtfile->execute();

            foreach ($stmtfile as $row) {
                $namefile = $row['file_prototype'];
                $mail->addAttachment("prototype_file/$namefile", $namefile);
            }
